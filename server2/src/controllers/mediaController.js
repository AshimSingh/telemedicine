...
const getMedia = async (req, res, next) => {
  try {
    ...
    const media = await Media.find(searchObj).skip(skip).limit(limit)
...
const updateMedia = async (req, res, next) => {
  try {
    ...
    const { id, title, caption, alt, type, description } = req.body
    ...
    if (mediaExist.length == 0) {
      ...
      if (result) {
        ...
        return errorResponse(res, 401, 'No media found')
      }
    }
  } catch (err) {
    next(err)
...
const deleteMedia = async (req, res, next) => {
  try {
    ...
    const { id } = req.body
    const value = await validateDeleteMedia(req.body)
