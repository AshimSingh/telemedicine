const {accountOpeningPayload,forgetPassPayload}= require('../src/helper/emailPayload')
...
describe('forget password payload',function(){
    it('test when required data is given',async function(){
        var res =  forgetPassPayload('test@test.com','https://loacalhost:3000/token','test')
        expect(res.params).to.have.property('name','test')
        expect(res.params).to.have.property('password','https://loacalhost:3000/token')
        expect(res).to.have.property('sender')
    })
    it('test when name is not given',async function(){
        var res =  forgetPassPayload('test@test.com','https://loacalhost:3000/token')
        expect(res.params).to.have.property('name', undefined)
        expect(res.params).to.have.property('password','https://loacalhost:3000/token')
        expect(res).to.have.property('sender')
    })
